...
public class DifangmeishiController extends BaseController {
    ...
    public String list() {
        ...
        List<Difangmeishi> list = difangmeishiService.selectPageExample(example, page, pagesize);   // 获取当前页的行数
    ...
    public String index() {
        ...
        List<Difangmeishi> list = difangmeishiService.selectPageExample(example, page, 12);

        assign("mapmeishifenlei5", new CommDAO().select("SELECT id,fenleimingcheng FROM meishifenlei"));
        assign("totalCount", request.getAttribute("totalCount"));
        assign("list", list);
        assign("where", where);
    ...
    public String updt() {
        ...
        // 获取行数据，并赋值给前台jsp页面
        Difangmeishi mmm = difangmeishiService.find(id);
        assign("mmm", mmm);
        assign("updtself", 0);
    ...
    public String insert() {
        ...
        difangmeishiService.insert(post); // 插入数据
    ...
    public String update() {
        ...
        difangmeishiService.update(post); // 更新数据
    ...
    public String detail() {
        _var = new LinkedHashMap(); // 重置数据
        int id = Request.getInt("id");
        Difangmeishi map = difangmeishiService.find(id);  // 根据前台url 参数中的id获取行数据
    ...
    public String detailweb() {
        _var = new LinkedHashMap(); // 重置数据
        int id = Request.getInt("id");
        Difangmeishi map = difangmeishiService.find(id);
        assign("map", map);
        return json();   // 将数据写给前端
    }
    ...
    public String delete() {
        ...
        difangmeishiService.delete(id);// 根据id 删除某行数据
